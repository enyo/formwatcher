...
(function() {
  ...
  Formwatcher = {
    ...
    getLabel: function(elements, automatchLabel) {
      ...
      if (!label && automatchLabel) {
        label = input.prev();
        if (!label.length || label.get(0).nodeName !== "LABEL" || label.attr("for")) {
          label = undefined;
        }
      }
      return label;
    ...
    setChanged: function(elements, watcher) {
      ...
      $.each(elements, function(index, element) {
        return element.addClass("changed");
      });
    ...
    unsetChanged: function(elements, watcher) {
      ...
      $.each(elements, function(index, element) {
        return element.removeClass("changed");
      });
    ...
    scanDocument: function() {
      ...
      return _.each(Formwatcher.options, function(options, formId) {
        return handleForm($("#" + formId));
      });
    },
  ...
  Formwatcher._ElementWatcher = (function() {
    ...
    function _ElementWatcher(watcher) {
      ...
      this.options = $.extend(true, {}, this.defaultOptions, (_ref = watcher.options[this.name]) != null ? _ref : {});
    }
  ...
  Watcher = (function() {
    ...
    function Watcher(form, options) {
      ...
      this.options = $.extend(true, {}, Formwatcher.defaultOptions, options || {});
      this.decorators = [];
      this.validators = [];
      ...
      $.each($(":input", this.form), function(i, input) {
        var elements, errorsElement, label, onchangeFunction, validateElementsFunction;
        input = $(input);
        if (!input.fwData("initialized")) {
          if (input.attr("type") === "hidden") {
            ...
            return $.each(elements, function() {
              ...
              return this.keyup(validateElementsFunction);
            });
          }
        }
      });
      ...
      $.each(submitButtons, function(i, element) {
        element = $(element);
    ...
    Watcher.prototype.enableForm = function() {
      return $(":input", this.form).prop("disabled", false);
    };

    Watcher.prototype.disableForm = function() {
      return $(":input", this.form).prop("disabled", true);
    };
    ...
    Watcher.prototype.submitAjax = function() {
      ...
      $.each($(":input", this.form), function(i, input) {
        input = $(input);
        if (input.fwData("forceSubmission") || input.attr("type") === "checkbox" || input.fwData("changed") || _this.options.submitUnchanged ? input.attr("type") !== "checkbox" || input.is(":checked") : void 0) {
          return fields[(input.attr("name") ? input.attr("name") : "unnamedInput_" + (i++))] = input.val();
        }
      });
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (!this.options.responseCheck(data)) {
              return this.callObservers("error", data);
            } else {
    ...
    Watcher.prototype.ajaxSuccess = function() {
      return _.each(this.allElements, _.bind(function(elements) {
        var isEmpty;
