...
task 'minify', 'minify and combine', -> minify -> log ":)", green
...
task 'publish', 'checkout master and publish to npm', -> publish -> log "B-D", green
...
#
# **given** optional function as callback
# **then** invoke launch passing minify command
minify = (callback) ->
  launch "#{__dirname}/minify.sh", [], callback
...
# **then** invoke launch passing docco command
publish = (callback) ->
  launch "#{__dirname}/npm_publish.sh", [], callback
